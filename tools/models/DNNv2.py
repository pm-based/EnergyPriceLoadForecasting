...
class DNNRegressorV2:
    ...
    def __build_model__(self, loss):
        ...
        x_ARX = (tf.keras.layers.Dense(self.settings['hidden_size_l1'],
                                   activation='linear',
                                   kernel_regularizer=tf.keras.regularizers.l1_l2(l1=self.settings['l1'],
                                                                                  l2=self.settings['l2'])
                                  )(x))

        x_2 = (tf.keras.layers.Dense(self.settings['hidden_size_l1'],
                                     activation=self.settings['activation_2'],
                                     kernel_regularizer=tf.keras.regularizers.l1_l2(l1=self.settings['l1'],
                                                                                    l2=self.settings['l2'])
                                     )(x))
        #x_2 = tf.keras.layers.Dropout(0.05)(x_2)
        x_2 = (tf.keras.layers.Dense(self.settings['hidden_size_l3'],
                                     activation=self.settings['activation_2'],
                                     kernel_regularizer=tf.keras.regularizers.l1_l2(l1=self.settings['l1'],
                                                                                    l2=self.settings['l2'])
                                     )(x_2))

        x = (tf.keras.layers.Dense(self.settings['hidden_size_l4'],
                                   activation=self.settings['activation_1'],
                                   kernel_regularizer=tf.keras.regularizers.l1_l2(l1=self.settings['l1'],
                                                                                  l2=self.settings['l2'])
                                  )(tf.concat([x_ARX, x_1, x_2], axis=1)))
    ...
    def get_hyperparams_trial(trial, settings):
        settings['hidden_size'] = trial.suggest_int('hidden_size', 64, 832, step=64)
        settings['n_hidden_layers'] = trial.suggest_int('n_hidden_layers', 1, 3)
        settings['lr'] = trial.suggest_float('lr', 2e-4, 1e-2, log=True)
