...
class CNNLSTMRegressor:
    ...
    def __build_model__(self, loss):
        ...
        # CNN
        x_cnn = (tf.keras.layers.Conv1D(self.settings['hidden_size'], kernel_size = 7,
        ...
        x_cnn = tf.keras.layers.Dense(128, activation=self.settings['activation'])(x_cnn)

        # LSTM
        x_lstm = tf.keras.layers.BatchNormalization()(x_in)
        for hl in range(self.settings['n_hidden_LSTM_layers'] - 1):
            x_lstm = tf.keras.layers.LSTM(self.settings['hidden_size'],
                                     # activation=self.settings['activation'],
        ...
        x_lstm = (tf.keras.layers.LSTM(128,
                                  # activation=self.settings['activation'],
    ...
    def get_hyperparams_searchspace(): # used only for grid search
        return {'kernel_size': [7], # 'kernel_size': 3, # 'kernel_size': 5, # 'kernel_size': 7,
                'hidden_size': [128, 512],
                'lr': [1e-4, 1e-3]}
